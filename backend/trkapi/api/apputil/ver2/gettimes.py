...
requests_cache.install_cache(
    cache_name='gettimes_cache', backend='sqlite', expire_after=180)
...
def gettimes(target):
    ...
    for time in times:
        ...
        if app_config.conf_app_date_simulation == "true":
            newtime3 = newtime2.replace(int(app_config.conf_app_simulation_date.split(", ")[0]), int(
                app_config.conf_app_simulation_date.split(", ")[1]), int(app_config.conf_app_simulation_date.split(", ")[2]))
